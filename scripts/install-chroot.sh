...
install_gpu_drivers() {
  ...
  for VENDOR in $GPU_CHOICE; do
    ...
    amd)
      xbps-install -Sy linux-firmware-amd \
        mesa-dri vulkan-loader mesa-vulkan-radeon mesa-opencl mesa-vaapi mesa-vdpau Vulkan-ValidationLayers
      ;;
    intel)
      xbps-install -Sy linux-firmware-intel \
        mesa-dri mesa-vulkan-intel mesa-opencl mesa-vaapi mesa-vdpau libva-glx Vulkan-ValidationLayers
...
desktop_setup() {
  # Install utilities
  xbps-install -Sy \
    clang go rust nodejs python3 lua openjdk21 php \
  ...
  xbps-install -Sy \
    ...
    dolphin-plugins xdg-user-dirs okular ark kwalletmanager \
    fuse samba ntfs-3g smbnetfs nfs-utils system-config-printer print-manager \
    demicode aha fwupd clinfo xdpyinfo mesa-demos glxinfo Vulkan-Tools power-profiles-daemon
